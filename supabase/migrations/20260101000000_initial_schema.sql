-- =============================================================================
-- 0001 · Initial schema
-- Baseline for ANMOL Art. Idempotent so it can run against a fresh database or
-- one already provisioned from the earlier supabase_migration_v2.sql.
-- Tables: categories, stock_images, site_config, hero_slides, services, faqs,
--         contact_submissions  (+ RLS, storage policies)
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS stock_images_category_idx ON stock_images (category);
CREATE INDEX IF NOT EXISTS stock_images_created_idx  ON stock_images (created_at DESC);
...
-- Public read for content tables; authenticated write. (Write policies are
-- tightened to role-based access in 0002.)
...
CREATE POLICY "Public read stock_images" ON stock_images FOR SELECT TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "Auth write stock_images"  ON stock_images;
CREATE POLICY "Auth write stock_images"  ON stock_images FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE POLICY "Auth write faqs"          ON faqs         FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Contact submissions: anyone may insert; only authenticated may read/manage.
DROP POLICY IF EXISTS "Public insert contact_submissions" ON contact_submissions;
CREATE POLICY "Public insert contact_submissions" ON contact_submissions FOR INSERT TO anon, authenticated WITH CHECK (true);
...
CREATE POLICY "Auth update contact_submissions" ON contact_submissions FOR UPDATE TO authenticated USING (true) WITH CHECK (true);
...
-- ── Storage bucket + policies (stock-images) ─────────────────────────────────
